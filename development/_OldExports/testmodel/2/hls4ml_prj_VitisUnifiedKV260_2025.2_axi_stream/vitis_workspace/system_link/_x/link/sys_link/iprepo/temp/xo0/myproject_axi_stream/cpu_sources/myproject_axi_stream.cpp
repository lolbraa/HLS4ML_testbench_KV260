...
void load_input(hls::stream<dma_data_packet> &axi_input_stream, hls::stream<input_t> &model_input_stream,
                int batch_size) {
load_input_loop:
    // send the data to the stream
    for (int q = 0; q < batch_size; q++) {
        for (unsigned chunk_idx = 0; chunk_idx < N_IN / input_t::size; ++chunk_idx) {
            input_t input_chunk;
            for (unsigned elem_idx = 0; elem_idx < input_t::size; elem_idx++) {
...
void compute( hls::stream<input_t> &model_input_stream, hls::stream<result_t> &model_output_stream,
    int batch_size) {
    for (int q = 0; q < batch_size; q++) {
...
void myproject_axi_stream(hls::stream<dma_data_packet> &axi_input_stream, hls::stream<dma_data_packet> &axi_output_stream,
    ...
    static hls::stream<input_t> model_input_stream("model_input");
