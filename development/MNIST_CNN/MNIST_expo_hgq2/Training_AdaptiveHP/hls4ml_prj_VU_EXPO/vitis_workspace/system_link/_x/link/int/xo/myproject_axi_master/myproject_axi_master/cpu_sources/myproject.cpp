...
void myproject(
    hls::stream<input_image_t> &input_image,
    hls::stream<result_t> &layer12_out
) {

    // hls-fpga-machine-learning insert IO
    #pragma HLS INTERFACE axis port=input_image,layer12_out 
    #pragma HLS DATAFLOW
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<dense0_weight_t, 4000>(w12, "w12.txt");
        nnet::load_weights_from_txt<dense0_bias_t, 10>(b12, "b12.txt");
        loaded_weights = true;    }
#endif
    // ****************************************
    ...
    nnet::conv_2d_cl<input_image_t, conv0_t, config3>(input_image, layer3_out, w3, b3); // conv0
    ...
    nnet::dense<pool1_t, result_t, config12>(layer10_out, layer12_out, w12, b12); // dense0
