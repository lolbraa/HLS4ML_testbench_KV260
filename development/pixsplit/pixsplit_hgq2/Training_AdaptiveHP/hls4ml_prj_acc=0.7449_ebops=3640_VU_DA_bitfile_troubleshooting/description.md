
    Description of HLS4ML-project.

    AdaptiveHP acc=0.7449 ebops=3640 Distributed Arithmetic

    - Bitfile: True
    - Environment: devenv-hgq+da (environment-HGQ+DA.yml)
    - Target Device: KV260 (xck26-sfvc784-2LV-c)
    - Dataset: Pixel Cluster Splitting
    - Vivado/Vitis: 2025.2
    - Model Architecture: pixsplit_hgq2
    - Model Revision: Training_AdaptiveHP
    - HLS4ML Revision: acc=0.7449_ebops=3640_VU_DA_bitfile_troubleshooting

    The model summary is in the parent-directory, `summary.txt`
    