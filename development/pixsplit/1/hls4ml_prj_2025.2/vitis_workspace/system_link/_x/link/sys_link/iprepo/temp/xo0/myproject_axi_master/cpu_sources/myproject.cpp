...
void myproject(
    hls::stream<input_layer_18_t> &input_layer_18,
    hls::stream<result_t> &layer15_out
) {

    // hls-fpga-machine-learning insert IO
    #pragma HLS INTERFACE axis port=input_layer_18,layer15_out 
    #pragma HLS DATAFLOW
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<encoder_layer_weight_t, 512>(w12, "w12.txt");
        nnet::load_weights_from_txt<encoder_layer_bias_t, 16>(b12, "b12.txt");
        nnet::load_weights_from_txt<dense_out_weight_t, 48>(w15, "w15.txt");
        nnet::load_weights_from_txt<dense_out_bias_t, 3>(b15, "b15.txt");
        loaded_weights = true;    }
#endif
    // ****************************************
    ...
    hls::stream<encoder_layer_t> layer12_out("layer12_out");
    #pragma HLS STREAM variable=layer12_out depth=1

    hls::stream<encoder_layer_relu_t> layer13_out("layer13_out");
    #pragma HLS STREAM variable=layer13_out depth=1

    nnet::dense<input_layer_18_t, dense_0_t, config3>(input_layer_18, layer3_out, w3, b3); // dense_0
    ...
    nnet::dense<dense_2_relu_t, encoder_layer_t, config12>(layer10_out, layer12_out, w12, b12); // encoder_layer

    nnet::relu<encoder_layer_t, encoder_layer_relu_t, relu_config13>(layer12_out, layer13_out); // encoder_layer_relu

    nnet::dense<encoder_layer_relu_t, result_t, config15>(layer13_out, layer15_out, w15, b15); // dense_out
