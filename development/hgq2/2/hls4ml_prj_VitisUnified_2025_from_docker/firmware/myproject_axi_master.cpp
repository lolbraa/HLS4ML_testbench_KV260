...
void compute(hls::stream<input_layer_1_t>& stream_in0_input_layer_1, hls::stream<result_t>& stream_out0_layer12_out, int batch_size) {
    for (int q = 0; q < batch_size; q++) {
    myproject(stream_in0_input_layer_1, stream_out0_layer12_out);
    }
}
...
void myproject_axi_master(
    ...
    #pragma HLS INTERFACE m_axi port=gmem_in0_ptr_input_layer_1 bundle=gmem_in0 depth=16
    ...
    load_input(gmem_in0_ptr_input_layer_1, stream_in0_input_layer_1, batch_size, 16);

    compute(stream_in0_input_layer_1, stream_out0_layer12_out, batch_size);
