...
void compute(hls::stream<input_t>& stream_in0_input_layer, hls::stream<result_t>& stream_out0_layer13_out, int batch_size) {
    for (int q = 0; q < batch_size; q++) {
    myproject(stream_in0_input_layer, stream_out0_layer13_out);
    }
}
...
void myproject_axi_master(
    ...
    #pragma HLS INTERFACE m_axi port=gmem_out0_ptr_layer13_out bundle=gmem_out0 depth=5
    ...
    compute(stream_in0_input_layer, stream_out0_layer13_out, batch_size);

    store_result(gmem_out0_ptr_layer13_out, stream_out0_layer13_out, batch_size, 5);
}
