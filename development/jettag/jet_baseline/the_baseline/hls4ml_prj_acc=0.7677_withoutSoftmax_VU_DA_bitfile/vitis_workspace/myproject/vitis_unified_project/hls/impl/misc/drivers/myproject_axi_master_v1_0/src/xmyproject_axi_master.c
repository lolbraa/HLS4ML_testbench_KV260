...
void XMyproject_axi_master_Set_gmem_out0_ptr_layer8_out(XMyproject_axi_master *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XMyproject_axi_master_WriteReg(InstancePtr->Control_BaseAddress, XMYPROJECT_AXI_MASTER_CONTROL_ADDR_GMEM_OUT0_PTR_LAYER8_OUT_DATA, (u32)(Data));
    XMyproject_axi_master_WriteReg(InstancePtr->Control_BaseAddress, XMYPROJECT_AXI_MASTER_CONTROL_ADDR_GMEM_OUT0_PTR_LAYER8_OUT_DATA + 4, (u32)(Data >> 32));
}

u64 XMyproject_axi_master_Get_gmem_out0_ptr_layer8_out(XMyproject_axi_master *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XMyproject_axi_master_ReadReg(InstancePtr->Control_BaseAddress, XMYPROJECT_AXI_MASTER_CONTROL_ADDR_GMEM_OUT0_PTR_LAYER8_OUT_DATA);
    Data += (u64)XMyproject_axi_master_ReadReg(InstancePtr->Control_BaseAddress, XMYPROJECT_AXI_MASTER_CONTROL_ADDR_GMEM_OUT0_PTR_LAYER8_OUT_DATA + 4) << 32;
    return Data;
}
