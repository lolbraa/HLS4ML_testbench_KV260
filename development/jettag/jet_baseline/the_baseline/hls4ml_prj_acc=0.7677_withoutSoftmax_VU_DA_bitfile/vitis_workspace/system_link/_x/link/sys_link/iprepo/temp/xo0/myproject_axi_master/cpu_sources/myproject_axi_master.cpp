...
void compute(hls::stream<input_t>& stream_in0_input_layer, hls::stream<result_t>& stream_out0_layer8_out, int batch_size) {
    for (int q = 0; q < batch_size; q++) {
    myproject(stream_in0_input_layer, stream_out0_layer8_out);
    }
}
...
void myproject_axi_master(
    ...
    #pragma HLS INTERFACE m_axi port=gmem_out0_ptr_layer8_out bundle=gmem_out0 depth=5
    #pragma HLS INTERFACE s_axilite port=batch_size bundle=control
    #pragma HLS INTERFACE s_axilite port=return bundle=control

    static hls::stream<input_t> stream_in0_input_layer;
    static hls::stream<result_t> stream_out0_layer8_out;

    #pragma HLS STREAM variable=stream_in0_input_layer depth=STREAM_BUF_IN_SZ
    #pragma HLS STREAM variable=stream_out0_layer8_out depth=STREAM_BUF_OUT_SZ

    #pragma HLS dataflow

    load_input(gmem_in0_ptr_input_layer, stream_in0_input_layer, batch_size, 16);

    compute(stream_in0_input_layer, stream_out0_layer8_out, batch_size);

    store_result(gmem_out0_ptr_layer8_out, stream_out0_layer8_out, batch_size, 5);
}
